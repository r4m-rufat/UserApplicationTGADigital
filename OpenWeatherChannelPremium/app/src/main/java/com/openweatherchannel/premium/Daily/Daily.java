package com.openweatherchannel.premium.Daily;
...
import com.openweatherchannel.premium.AdapTers.ActivityDailyAdapter;
...
public class Daily extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private TextView daily_forecast;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (preferenceManager.getString("for_reload_daily") != null) {
    ...
    private void dailyResponse(String lat, String lon) {
        ...
        final Call<WeeklyResult> weeklyResultCall = ManagerAll.getInstance().getWeatherWeeklyInformations(lat, lon, "hourly,daily", units, language_api, api_key);

        weeklyResultCall.enqueue(new Callback<WeeklyResult>() {
            @Override
            public void onResponse(Call<WeeklyResult> call, Response<WeeklyResult> response) {
                if (response.isSuccessful()) {
                    ...
                    preferenceManager.putString("for_reload_daily", weeklyResult.getCurrent().getWeather().get(0).getDescription());

                    ActivityDailyAdapter adapter = new ActivityDailyAdapter(context, weeklyResult.getDaily(), preferenceManager);
    ...
    private void currentData(String city_name) {
        ...
        call.enqueue(new Callback<CurrentResult>() {
            ...
            public void onFailure(Call<CurrentResult> call, Throwable t) {
                ...
                dailyResponse(preferenceManager.getString("lat"), preferenceManager.getString("lon"));
    ...
    private void setupWidgets() {
        ...
        dailyRecyclerView = findViewById(R.id.recycler_view_forActivityDaily);
        dailyRecyclerView.setHasFixedSize(false);
        dailyRecyclerView.setLayoutManager(new LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false));
    ...
    private void setupBottomNavigationViewEx() {
        ...
        bottomNavigationView.setSelectedItemId(R.id.id_daily);
    ...
    private void loadWeeklyData() {
        ...
        ActivityDailyAdapter activityDailyAdapter = new ActivityDailyAdapter(context, dailyItemList, preferenceManager);

        dailyRecyclerView.setAdapter(activityDailyAdapter);

        activityDailyAdapter.notifyDataSetChanged();
    ...
    protected void onStart() {
        ...
        daily_forecast.setText(getString(R.string.daily_forecast));
